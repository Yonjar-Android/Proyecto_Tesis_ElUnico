...
export const descargarReporteVentasServicioPorPeriodo = async (req: Request, res: Response) => {
    try {
        const { search = "", fechaInicio = "", fechaFin = "" } = req.query;

        const excelBuffer = await generateReporteVentasServicioPorPeriodoExcel(
            search as string,
            fechaInicio as string,
            fechaFin as string
        );
        ...
        const fechaActual = new Date().toISOString().split('T')[0];
        const filename = `reporte_ventas_servicio_${fechaInicio || 'inicio'}_a_${fechaFin || fechaActual}.xlsx`;
        ...
        console.error('Error generando Excel de ventas por servicio:', error);
        res.status(500).json({
            success: false,
...
export const descargarReporteVentasProductoPorPeriodo = async (req: Request, res: Response) => {
    try {
        const { search = "", fechaInicio = "", fechaFin = "" } = req.query;

        const excelBuffer = await generateReporteVentasProductoPorPeriodoExcel(
            search as string,
            fechaInicio as string,
            fechaFin as string
        );
        ...
        const fechaActual = new Date().toISOString().split('T')[0];
        const filename = `reporte_ventas_producto_${fechaInicio || 'inicio'}_a_${fechaFin || fechaActual}.xlsx`;
        ...
        console.error('Error generando Excel de ventas por producto:', error);
        res.status(500).json({
            success: false,
...
export const descargarReporteInventario = async (req: Request, res: Response) => {
    try {
        const { search = "", Id_categoria = null, Id_marca = null } = req.query;

        const excelBuffer = await generateReporteInventarioExcel(
            search as string,
        ...
        const fechaActual = new Date().toISOString().split('T')[0];
        const filename = `reporte_inventario_${fechaActual}.xlsx`;
